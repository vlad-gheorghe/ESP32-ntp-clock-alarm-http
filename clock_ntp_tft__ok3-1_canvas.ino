...
*/
 ...
 void drawClock() {
  ...
  canvas.setTextColor(0xFA60); //portocaliu intens
...
*/
void drawInfoScreen1() {
  // 1. Curățăm ecranul virtual din RAM (instantaneu, în fundal)
  canvas.fillScreen(ST77XX_BLACK);
  
  // 2. Activăm fontul modern (FreeSans9pt7b) pe canvas
  canvas.setFont(&FreeSans9pt7b);
  canvas.setTextSize(1); // Mărimea 1 este ideală pentru acest font pe 240x135
  
  // La fonturile custom, axa Y reprezintă linia de bază (baseline), 
  // așa că începem de la y = 20 pentru ca textul să nu iasă din ecran
  int y = 20; 
  int x = 15; // Marginea din stânga

  // Titlu pagină (Portocaliu aprins)
  canvas.setCursor(x, y);
  canvas.setTextColor(0xFA60); 
  canvas.print("INFO 1/2");
  y += 24; // Spațiere verticală perfectă pentru fontul de 9pt

  // Ziua curentă (Alb)
  canvas.setCursor(x, y);
  canvas.setTextColor(ST77XX_WHITE);
  canvas.print("Day: ");
  canvas.print(weekdayStr);
  y += 24;

  // Stare Alarmă (Eticheta albă, valoarea colorată)
  canvas.setCursor(x, y);
  canvas.setTextColor(ST77XX_WHITE);
  canvas.print("Alarm: ");
  if (myAlarm.active) {
    canvas.setTextColor(ST77XX_RED);
    if (myAlarm.hour < 10) canvas.print("0");
    canvas.print(myAlarm.hour);
    canvas.print(":");
    if (myAlarm.minute < 10) canvas.print("0");
    canvas.print(myAlarm.minute);
  } else {
    canvas.setTextColor(0x7BEF); // Gri închis/stins pentru "OFF"
    canvas.print("OFF");
  }
  y += 24;

  // Stare Timer (Eticheta albă, valoarea Cyan dacă rulează)
  canvas.setCursor(x, y);
  canvas.setTextColor(ST77XX_WHITE);
  canvas.print("Timer: ");
  if (timerActive) {
    ...
    uint64_t remaining = (elapsed >= timerDurationUs) ? 0 : (timerDurationUs - elapsed);
    int secRemaining = (remaining + 500000) / 1000000;
    canvas.print(secRemaining);
    canvas.print(" sec");
  } else {
    canvas.setTextColor(0x7BEF); // Gri
    canvas.print("OFF");
  }
  y += 24;

  // Stare WiFi (Eticheta albă, valoarea Verde/Roșu)
  canvas.setCursor(x, y);
  canvas.setTextColor(ST77XX_WHITE);
  canvas.print("WiFi: ");
  if (WiFi.status() == WL_CONNECTED) {
  ...
  // 3. IMPULS HARDWARE: Trimitem bufferul complet din RAM direct pe ecran
  // Această comandă elimină 100% orice urmă de pâlpâire
  tft.drawRGBBitmap(0, 0, canvas.getBuffer(), 240, 135);
  
  // Resetăm fontul implicit la final pentru a nu afecta alte funcții ale ecranului fizic
  tft.setFont(); 
}
...
*/
void drawInfoScreen2() {
  // 1. Curățăm ecranul virtual din RAM (fără pâlpâire)
  canvas.fillScreen(ST77XX_BLACK);
  ...
  // y reprezintă linia de bază (baseline) a textului pentru fontul custom
  int y = 20;
  int x = 15;
  ...
  canvas.setCursor(x, y);
  canvas.setTextColor(0xFA60);
  canvas.print("INFO 2/2");
  y += 24; // Spațiere verticală calibrată

  // Nume rețea WiFi (SSID)
  canvas.setCursor(x, y);
  canvas.setTextColor(ST77XX_WHITE);
  String ssidDisplay = wifiSSID;
  ...
  canvas.print("Time: ");
  if (timeValid) {
    canvas.setTextColor(0x07FF); // Cyan pentru SYNC
    canvas.print("SYNC");
  } else {
    canvas.setTextColor(ST77XX_YELLOW);
    canvas.print("NO SYNC");
  }
  y += 24;
  ...
  canvas.setTextColor(ST77XX_WHITE);
  canvas.print("RAM: ");
  canvas.print(esp_get_free_heap_size() / 1024);
  canvas.print(" KB");

  // 3. IMPULS HARDWARE: Trimitem bufferul complet din RAM direct pe ecran
  tft.drawRGBBitmap(0, 0, canvas.getBuffer(), 240, 135);
  
  // Resetăm fontul implicit la final
  tft.setFont(); 
}

// ================= SYSTEM FUNCTIONS =================
void showSplash() {
  // Ștergem ecranul cu fundal negru la pornire
  tft.fillScreen(ST77XX_BLACK);
  tft.setFont(); // Folosim fontul standard mărit pentru splash screen
  tft.setTextSize(3);
  ...
  // Al doilea rând (Portocaliu aprins)
  tft.setTextColor(0xFA60); 
  tft.setCursor(70, 75);
  tft.print("CLOCK");
  
  // Menținem logo-ul vizibil scurt timp, apoi curățăm ecranul pentru meniu
  delay(1000); 
  tft.fillScreen(ST77XX_BLACK);
}
// ================= SYSTEM FUNCTIONS =================
